...
CCSocketServerDlg::CCSocketServerDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CCSocketServerDlg::IDD, pParent)
{
...
void CCSocketServerDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	//  DDX_Control(pDX, IDC_Exit, m_MSG);
	DDX_Control(pDX, IDC_EDIT1, m_MSG);
}

BEGIN_MESSAGE_MAP(CCSocketServerDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_Create, &CCSocketServerDlg::OnBnClickedCreate)
	ON_BN_CLICKED(IDC_Send, &CCSocketServerDlg::OnBnClickedSend)
	ON_BN_CLICKED(IDC_Exit, &CCSocketServerDlg::OnBnClickedExit)
END_MESSAGE_MAP()


// CCSocketServerDlg 消息处理程序
...
void CCSocketServerDlg::OnBnClickedCreate()
{
m_clientSocket.ShutDown(2);
m_clientSocket.m_hSocket=INVALID_SOCKET;
AfxSocketInit();
BOOL bFlag=m_clientSocket.Create(1088,SOCK_DGRAM,NULL);
      if(!bFlag)
      { 
...
void CCSocketServerDlg::OnBnClickedSend()
{
	// TODO: 在此添加控件通知处理程序代码
 m_clientSocket.AsyncSelect(FD_READ);
...
void CCSocketServerDlg::OnBnClickedExit()
{
//关闭Socket
m_clientSocket.ShutDown(2);
//关闭对话框
